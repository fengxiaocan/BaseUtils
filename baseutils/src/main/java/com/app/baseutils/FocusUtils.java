    ...
    public static void focusView(final View focusBox, final View newFocus, final float outSide) {
        ...
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                focusBox.layout(
                        (int) (focusBox_left + (newFocus.getLeft() - outSide - focusBox_left) * f),
                        (int) (focusBox_top + (newFocus.getTop() - outSide - focusBox_top) * f),
                        (int) (focusBox_right +
                               (newFocus.getRight() + outSide - focusBox_right) * f),
                        (int) (focusBox_bottom +
                               (newFocus.getBottom() + outSide - focusBox_bottom) * f));
                focusBox.setX(focusBox_x + (newFocus.getX() - outSide + getParentX(newFocus, 0) -
                                            focusBox_x) * f);
                focusBox.setY(focusBox_y + (newFocus.getY() - outSide + getParentY(newFocus, 0) -
                                            focusBox_y) * f);
                if (f == 1) {
