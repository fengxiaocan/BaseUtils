...
public final class SpanUtils {
	...
	public static SpanUtils getSpan(){
	...
	public SpanUtils setLineHeight(
			@IntRange(from = 0) final int lineHeight,@Align final int align)
	{
	...
	public SpanUtils setQuoteColor(
			@ColorInt final int color,@IntRange(from = 1) final int stripeWidth,
	...
	public SpanUtils setLeadingMargin(
			@IntRange(from = 0) final int first,@IntRange(from = 0) final int rest)
	{
		this.first = first;
		this.rest = rest;
		return this;
	...
	public SpanUtils setBullet(
			@ColorInt final int color,@IntRange(from = 0) final int radius,
			@IntRange(from = 0) final int gapWidth)
	{
	...
	public SpanUtils setBlur(
			@FloatRange(from = 0,
					fromInclusive = false) final float radius,final Blur style)
	{
	...
	public SpanUtils setShadow(
			@FloatRange(from = 0,
					fromInclusive = false) final float radius,final float dx,final float dy,
	...
	private void updateCharCharSequence() {
		...
		if (quoteColor != COLOR_DEFAULT) {
			mBuilder.setSpan(new CustomQuoteSpan(quoteColor,stripeWidth,quoteGapWidth),start,end,
			                 flag);
		}
		if (bulletColor != COLOR_DEFAULT) {
			mBuilder.setSpan(new CustomBulletSpan(bulletColor,bulletRadius,bulletGapWidth),start,
			                 end,flag);
		}
		if (fontSize != -1) {
		...
		if (blurRadius != -1) {
			mBuilder.setSpan(new MaskFilterSpan(new BlurMaskFilter(blurRadius,style)),start,end,
			                 flag);
		}
		if (shader != null) {
		...
		if (shadowRadius != -1) {
			mBuilder.setSpan(new ShadowSpan(shadowRadius,shadowDx,shadowDy,shadowColor),start,end,
			                 flag);
		}
		if (spans != null) {
	...
	protected class CustomLineHeightSpan extends CharacterStyle implements LineHeightSpan {
		...
		public void chooseHeight(
				final CharSequence text,final int start,final int end,final int spanstartv,
	...
	class SpaceSpan extends ReplacementSpan {
		...
		@Override
		public int getSize(
				@NonNull final Paint paint,final CharSequence text,
		...
		@Override
		public void draw(
				@NonNull final Canvas canvas,final CharSequence text,
	...
	class CustomQuoteSpan implements LeadingMarginSpan {
		...
		public void drawLeadingMargin(
				final Canvas c,final Paint p,final int x,final int dir,final int top,
				final int baseline,final int bottom,final CharSequence text,final int start,
				final int end,final boolean first,final Layout layout)
		{
	...
	class CustomBulletSpan implements LeadingMarginSpan {
		...
		public void drawLeadingMargin(
				final Canvas c,final Paint p,final int x,final int dir,final int top,
				final int baseline,final int bottom,final CharSequence text,final int start,
				final int end,final boolean first,final Layout l)
		{
	...
	protected class CustomImageSpan extends CustomDynamicDrawableSpan {
		...
		public Drawable getDrawable() {
			...
			if (mDrawable != null) {
				...
				Bitmap bitmap;
				try {
					InputStream is = Utils.getContext().getContentResolver()
					                      .openInputStream(mContentUri);
	...
	protected abstract class CustomDynamicDrawableSpan extends ReplacementSpan {
		...
		@Override
		public int getSize(
				@NonNull final Paint paint,final CharSequence text,final int start,final int end,
		...
		@Override
		public void draw(
				@NonNull final Canvas canvas,final CharSequence text,final int start,final int end,
	...
	protected class ShaderSpan extends CharacterStyle implements UpdateAppearance {
	...
	protected class ShadowSpan extends CharacterStyle implements UpdateAppearance {
