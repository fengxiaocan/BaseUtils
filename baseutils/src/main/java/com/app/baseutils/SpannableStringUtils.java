...
public final class SpannableStringUtils {
	...
	public static class Builder {
		...
		private void setSpan() {
			...
			if (quoteColor != defaultValue) {
				mBuilder.setSpan(new CustomQuoteSpan(quoteColor,stripeWidth,quoteGapWidth),start,
				                 end,flag);
				quoteColor = defaultValue;
			}
			if (isBullet) {
				mBuilder.setSpan(new CustomBulletSpan(bulletColor,bulletRadius,bulletGapWidth),
				                 start,end,flag);
				isBullet = false;
			}
			if (fontSize != -1) {
			...
			if (imageIsBitmap || imageIsDrawable || imageIsUri || imageIsResourceId) {
				if (imageIsBitmap) {
					mBuilder.setSpan(new CustomImageSpan(Utils.getContext(),bitmap,align),start,end,
					                 flag);
					bitmap = null;
					imageIsBitmap = false;
				}
					...
					mBuilder.setSpan(new CustomImageSpan(Utils.getContext(),uri,align),start,end,
					                 flag);
					uri = null;
					imageIsUri = false;
				}
				else {
					mBuilder.setSpan(new CustomImageSpan(Utils.getContext(),resourceId,align),start,
					                 end,flag);
					resourceId = 0;
	...
	static class MarginSpan extends ReplacementSpan {
		...
		@Override
		public int getSize(
				@NonNull Paint paint,CharSequence text,@IntRange(from = 0) int start,
		...
		@Override
		public void draw(
				@NonNull Canvas canvas,CharSequence text,@IntRange(from = 0) int start,
	...
	static class CustomQuoteSpan implements LeadingMarginSpan {
		...
		public void drawLeadingMargin(
				Canvas c,Paint p,int x,int dir,int top,int baseline,int bottom,CharSequence text,
	...
	static class CustomImageSpan extends CustomDynamicDrawableSpan {
		...
		CustomImageSpan(Context context,Bitmap b,int verticalAlignment) {
			...
			mContext = context;
			mDrawable = context != null ? new BitmapDrawable(context.getResources(),b)
	...
	static abstract class CustomDynamicDrawableSpan extends ReplacementSpan {
				...
				@NonNull Paint paint,CharSequence text,int start,int end,Paint.FontMetricsInt fm)
		{
			...
			final int fontHeight = (int)(paint.getFontMetrics().descent - paint
		...
		@Override
		public void draw(
				@NonNull Canvas canvas,CharSequence text,int start,int end,float x,int top,int y,
