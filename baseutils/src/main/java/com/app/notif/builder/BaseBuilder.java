...
public class BaseBuilder{
    ...
    //最基本的ui
    public int          smallIcon;//顶部状态栏的小图标
    ...
    public BaseBuilder addBtn(int icon,CharSequence text,
    ...
    public static class BtnActionBean{
        ...
        public BtnActionBean(int icon,CharSequence text,
                             PendingIntent pendingIntent){
            this.icon = icon;
            this.text = text;
    ...
    public BaseBuilder setBase(int icon,CharSequence contentTitle,
    ...
    public void build(){
        cBuilder = new NotificationCompat.Builder(NotifyUtils.context);
        ...
        if(bigIcon > 0){
            cBuilder.setLargeIcon(BitmapFactory.decodeResource(
                    NotifyUtils.context.getResources(),bigIcon));
        }
    ...
    public void show(){
        ...
        NotifyUtils.notify(id,notification);
    }
